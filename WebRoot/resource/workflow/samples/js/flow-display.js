function FlowDisplay(diagramDiv) {
     ...
     * 动画显示流程路径状态
     */
    this.animateFlowPath = function(strhistoryNodes,strStepKeys, isCompleted) {
    	
    	var historyNodes = strhistoryNodes.split(',');
        var stepKeys = strStepKeys.split(',');
        // 查找所有【已完成】步骤：【开始】-> 【已完成】（N个）
        var steps = findFinishedSteps(stepKeys, isCompleted);
        var finishSteps = findFinishedSteps(historyNodes, isCompleted);
        // 高亮所有“已完成”步骤
        ...
        if (!isCompleted) {
        	//已处理步骤，加闪烁动画
        	 for (var i = 0; i < historyNodes.length; i++) {
                 var stepKey = historyNodes[i];
                 var step = _displayer.findNodeForKey(stepKey);
                 loopRunningNode(step);
             }
            // “待处理”步骤，加上闪烁动画
        ...
        // 在连线上加闪烁动画
        var links = findFinishedLinks(finishSteps);
        loopLinks(links);
    };
    /** --------public method-------------end---------------------------**/
    ...
    function makeLinkTemplate() {
    	...
    	/* return G(go.Link, // the whole link panel
    	            { selectable: false, selectionAdornmentTemplate: makeLinkSelectionAdornmentTemplate() }, 
    ...
    function findFinishedLinks(steps) {
        ...
        for (var i = 0; i < steps.length; i++) {
            ...
            if(i<steps.length-1){
            	 // 连线
            	var node=steps[i].je;
                var link=_displayer.findLinksByExample({from:steps[i].je.key, to:steps[i+1].je.key});
