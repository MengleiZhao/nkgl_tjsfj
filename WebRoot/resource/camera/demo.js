...
$(function () {
	...
	if (-2 == iRet) {
        ...
        if(confirm('您还未安装过插件，请下载WebComponentsKit.exe安装，确认下载吗？（如果已经安装，请确认浏览器版本：仅支持32位浏览器：Firefox30~Firefox51、IE6~IE11、Chrome31~Chrome44、Safari5.0.2+，需要浏览器支持NPAPI）')){
        	window.open(base+"/resource/download/WebComponentsKit.exe");
        }
		return;
    }
	// 初始化插件参数及插入插件
	...
	if (-1 == WebVideoCtrl.I_CheckPluginVersion()) {
		...
		if(confirm('检测到新的插件版本，请下载WebComponentsKit.exe安装，确认下载吗？')){
        	window.open(base+"/resource/download/WebComponentsKit.exe");
        }
		return;
	}
