...
//初始化datagrid点击事件
	$('#hotestd_choose_dg').datagrid({
		onDblClickRow:function(index,row){
			...
			if(tabId=='reimbursein_hoteltab'){//差旅
				//城市名称
				var locationCity = $('#reimbursein_hoteltab').datagrid('getEditor',{
					index:rowindex,
					field:'locationCity'
				});
				$(locationCity.target).textbox('setValue', row.area);
				//城市id
				var cityId = $('#reimbursein_hoteltab').datagrid('getEditor',{
					index:rowindex,
					field:'cityId'
				});
				$(cityId.target).textbox('setValue', row.id);
			}else{
				//城市名称
				var locationCity = $('#hoteltab').datagrid('getEditor',{
					index:rowindex,
					field:'locationCity'
				});
				$(locationCity.target).textbox('setValue', row.area);
				//城市id
				var cityId = $('#hoteltab').datagrid('getEditor',{
					index:rowindex,
					field:'cityId'
				});
				$(cityId.target).textbox('setValue', row.id);
