	...
	<div  class="easyui-layout" fit="true" >
		...
		<table cellpadding="5" cellspacing="0" class="a_search_table" border="0" >
				<tr>
					...
					<td width="100px">
						<input id="WF_nameAndDept_name" name="" style="width: 90px;height:25px;" class="easyui-textbox"></input>
					</td>
					<th width="90px" class="queryth">部门代码：</th> 
					<td width="90px">
						<input id="WF_nameAndDept_code" name=""  style="width: 90px;height:25px;" class="easyui-textbox"></input>
					</td>
					<td >
		...
		<div  region="center" border="false">
			...
			data-options="collapsible:true,url:'${base}/wflow/NameAndDepart',
			...
			selectOnCheck: true,checkOnSelect:true,remoteSort:true,nowrap:false,singleSelect: false,striped: true" >
				<thead>
					<tr>
						...
						<th data-options="field:'name',align:'center',resizable:false,sortable:true" width="48%">部门</th>
						<th data-options="field:'code',align:'center',resizable:false,sortable:true" width="50%">部门编码</th>
					</tr>
				</thead>
			</table>
		</div>
...
$('#allcoa_nameAndDept_dg').datagrid({
	onDblClickRow:function(index,row){
		...
		if (row != null && selections.length == 1) {
			$("#TPD_departName").textbox('setValue',row.name); 
			$("#TPD_departCode").val(row.id); 
			closeFirstWindow();
		} else {
...
function queryCF(){  
	$('#allcoa_nameAndDept_dg').datagrid('load',{ 
		name:$('#WF_nameAndDept_name').textbox('getValue'),
		code:$('#WF_nameAndDept_code').textbox('getValue'),
	} ); 
}
//时间格式化
