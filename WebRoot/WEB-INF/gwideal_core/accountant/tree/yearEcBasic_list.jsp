...
<div class="easyui-layout" fit="true">
	<div data-options="region:'west',split:false"  style="width:200px;" >
		<ul id="EconomicTree" class="easyui-tree" data-options="url:'${base}/accoTree/tree',animate:true,lines:true" ></ul>
	</div>
	<div data-options="region:'center'" style="background-color: #f0f5f7">
	  <div style="height: 10px;background-color:#f0f5f7 "></div>
		...
		<table id="Economic_dg" border="0" class="easyui-datagrid"
				data-options="singleSelect:true,collapsible:true,url:'${base}/accoTree/JsonPagination',
...
function kmlx(val, row){
	...
	function view(){
		...
		if(null!=row && selections.length==1){
		    var win=creatWin('查看-经济分类科目',850,250,'icon-edit','/accoTree/view/'+row.fid);
		    win.window('open');
		}else{
	...
	function deleteEconomic(){
		var node=$('#EconomicTree').tree('getSelected');
		var row =$('#Economic_dg').datagrid('getChecked');
		var selections = $('#Economic_dg').datagrid('getSelections');
		var da=$('#Economic_dg').datagrid('getChecked');
		var fids=[15];
		var str;
		...
		if(null!=row){
			$.messager.confirm('系统提示','确认删除吗?',function(r){
				if(r){
					$.ajax({ 
						type: 'POST', 
						url: '${base}/accoTree/delete/'+row[0].fid,
						dataType: 'json',  
						data:{'fid':row[0].fid},
						success: function(data){ 
							if(data.success){
	...
	function addEconomic(){
		var node=$('#EconomicTree').tree('getSelected');
		//var win=creatWin('新增-经济分类科目',750,250,'icon-add','/accoTree/add');
		if(null!=node){
			win=creatWin('新增-经济分类科目',707,380,'icon-search','/accoTree/add?departId='+node.id);
		}
		win.window('open');
	}
	function editUser(){
		...
		var selections = $('#Economic_dg').datagrid('getSelections');
		if(null!=row && selections.length==1){
		    var win=creatWin('修改-经济分类科目',740,580,'icon-search','/accoTree/edit?departId='+row.fid);
		    win.window('open');
		}else{
			 $.messager.alert('系统提示','请选择一条数据！','info');  
		}
