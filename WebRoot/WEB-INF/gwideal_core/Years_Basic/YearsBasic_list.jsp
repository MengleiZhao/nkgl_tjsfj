...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
		...
		<table id="yb_dg" border="0" class="easyui-datagrid"
				data-options="singleSelect:true,collapsible:true,url:'${base}/YearsBsaic/JsonPagination',
...
function CZ(val, row) {
	return '<a href="#" onclick="detail(' + row.fbId
...
function showH(obj){
	...
	function view(){
		...
		if(null!=row && selections.length==1){
		    var win=creatWin('查看-经济分类科目',850,250,'icon-edit','/YearsBsaic/view/'+row.fid);
		    win.window('open');
		}else{
		...
		if(null!=row && selections.length==1){
			...
			$.messager.confirm('系统提示','确认删除用户吗?',function(r){
				if(r){
					$.ajax({ 
						type: 'POST', 
						url: '${base}/YearsBsaic/delete/'+row.fbId,
						dataType: 'json',  
						success: function(data){ 
							if(data.success){
								$.messager.alert('系统提示',data.info,'info');
	...
	function detail(id) {
		var win = creatWin('查看', 650, 300, 'icon-search','/YearsBsaic/detail/' + id);
		win.window('open');
	}	
	...
	function addYb(){
		var win=creatWin('新增-年度模板管理',650,300,'icon-search','/YearsBsaic/add');
		win.window('open');
	}
	function editYb(id){
		var win=creatWin('修改-年度模板管理',650,300,'icon-search','/YearsBsaic/edit/'+id);
		win.window('open');
	}
	function copymodel(id){
		var win=creatWin('另存-年度模板管理',650,300,'icon-search','/YearsBsaic/copymodel/'+id);
		win.window('open');
	}
	$('#userDepartTree').tree({
