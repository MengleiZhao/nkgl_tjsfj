...
.trbody{height: 30px;}
	...
	function deleteAttac(a){
		...
		function per1AddEditForm(){
			/* //附件的路径地址
			var s="";
			$(".fileUrl").each(function(){
				s=s+$(this).attr("id")+",";
			}); */
			$('#per1AddEditForm').form('submit', {
				onSubmit: function(){ 
    ...
    <div class="easyui-layout" fit="true" style="color: #333333">
   		<div data-options="region:'west',split:true"style="width:600px;border-color: dce5e9" id="westDiv">
	    	<form id="per1AddEditForm" action="${base}/PerformanceIndicator/save?leixing=1" method="post" data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
	    		<input type="hidden" name="fPerId1" id="per_fPerId1" value="${bean.fPerId1}"/>
				<table >
				<tr>
					<td >
						<div class="easyui-accordion" style="width:555px;margin-left: 20px;">
							<div title="一级绩效指标" data-options="iconCls:'icon-xxlb',collapsed:false,collapsible:false" style="overflow:auto;margin-top:10px;">
								<table class="ourtable" cellpadding="0" cellspacing="0">
									<tr class="trbody">
										<td class="td1"><span class="style1">*</span>一级绩效指标名称</td>
										<td class="td2">
											<input class="easyui-textbox"  id="per_fPerName1" name="fPerName1"  data-options="validType:'length[1,20]'" style="width: 150;height:25px" value="${bean.fPerName1}"/>
										</td>
										<td style="width: 50px">&nbsp;</td>
										<td class="td1"><span class="style1">*</span>一级绩效指标代码</td>
										<td class="td2">
											<input class="easyui-textbox" id="per_fPerCode1" name="fPerCode1" readonly="readonly" data-options="validType:'length[1,20]'" style="width: 150;height:25px" value="${bean.fPerCode1 }"/>
										</td>
									</tr>
								</table>	
							</div>
			...
			<div data-options="region:'south'" style="height: 51px;width: 100%; border: 0px;">
					<div style="text-align: center;">
						<c:if test="${openType=='add'||openType=='edit'}">
							<a href="javascript:void(0)" onclick="per1AddEditForm();">
