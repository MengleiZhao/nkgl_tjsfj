...
.trbody{height: 30px;}
	...
	function deleteAttac(a){
		...
		function expenditure1AddEditForm(){
			/* //附件的路径地址
			var s="";
			$(".fileUrl").each(function(){
				s=s+$(this).attr("id")+",";
			}); */
			$('#expenditure1AddEditForm').form('submit', {
				onSubmit: function(){ 
   				...
   				success:function(data){
   					...
   					if(data.success){
   						...
   						closeWindow();
   						$('#expenditure1AddEditForm').form('clear');
    ...
    <div class="easyui-layout" fit="true" style="color: #333333">
   		<div data-options="region:'west',split:true"style="width:600px;border-color: dce5e9" id="westDiv">
	    	<form id="expenditure1AddEditForm" action="${base}/ExpenditureBaseic/save?leixing=1" method="post" data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
	    		<input type="hidden" name="tEbId" id="expenditure_tEbId" value="${bean.tEbId}"/>
				<table >
				<tr>
					<td >
						<div class="easyui-accordion" style="width:555px;margin-left: 20px;">
							<div title="支出明细事项" data-options="iconCls:'icon-xxlb',collapsed:false,collapsible:false" style="overflow:auto;margin-top:10px;">
								<table class="ourtable" cellpadding="0" cellspacing="0">
									<tr class="trbody">
										<td class="td1"><span class="style1">*</span>费用类型(名称)</td>
										<td class="td2">
											<input class="easyui-textbox" id="expenditure_tEbName1" name="tEbName1"  data-options="validType:'length[1,20]'" style="width: 150;height:25px" value="${bean.tEbName1 }"/>
										</td>
										<td class="td4">&nbsp;</td>
										<td class="td1"><span class="style1">*</span>费用类型代码</td>
										<td class="td2">
											<input class="easyui-textbox"  id="expenditure_tEbCode1" name="tEbCode1"  data-options="validType:'length[1,20]'" style="width: 150;height:25px" value="${bean.tEbCode1}"/>
										</td>
									</tr>
								</table>	
							</div>
			...
			<div data-options="region:'south'" style="height: 51px;width: 100%; border: 0px;">
					<div style="text-align: center;">
						<c:if test="${openType=='add'||openType=='edit'}">
							<a href="javascript:void(0)" onclick="expenditure1AddEditForm();">
