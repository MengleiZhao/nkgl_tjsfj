...
<thead>
	<tr>
		...
		<th data-options="field:'fbiddingName',align:'center',editor:'textbox'" style="width: 30%"><span class="style1">*</span>中标商名称</th>
		<th data-options="field:'fbidAmount',align:'center',formatter:function(value,row,index){return fomatMoney(value,2);},editor:{type:'numberbox',options:{precision:2,editable:true}}" style="width: 20%"><span class="style1">*</span>中标金额(元)</th>
...
<%-- <div id="winningDivId">
		...
		<thead>
			<tr>
				...
				<th data-options="field:'fmName',width:185,align:'center',editor:{type:'textbox',options:{editable:false,prompt: '请点击此处选择',icons:[{iconCls:'icon-add',handler: function(e){
	...
	if(num==0){
			...
			if(fpItemsNames=='PMMC-4' || fpItemsNames=='PMMC-5'){
				$('#purchasing_tab_pur_'+i).datagrid({
				    columns:[[
						...
						{field:'fmName',title:'商品名称',width:385,align:'center',editor:{type:'textbox',options:{editable:false,prompt: '请点击此处选择',icons:[{iconCls:'icon-add',handler: function(e){
							var id =e.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[2].id;// e.target表示被点击的目标
						     var row = $("#"+id).datagrid('getSelected');
						     var index = $("#"+id).datagrid('getRowIndex',row);
						     selectCGQDDetail(index,id,row.fbiddingCode);
				...
				$('#purchasing_tab_pur_'+i).datagrid({
				    columns:[[
						...
						{field:'mainId',hidden:true},
						{field:'fbiddingCode',hidden:true},
						{field:'fmName',title:'商品名称',width:185,align:'center',editor:{type:'textbox',options:{editable:false,prompt: '请点击此处选择',icons:[{iconCls:'icon-add',handler: function(e){
							var id =e.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[2].id;// e.target表示被点击的目标
						     var row = $("#"+id).datagrid('getSelected');
						     var index = $("#"+id).datagrid('getRowIndex',row);
						     selectCGQDDetail(index,id,row.fbiddingCode);
		...
		if(nums>num){
			...
			for (var i = num; i <nums; i++) {
				$("#toolbar"+i).remove();
				$('#winningDivId').append('<div id="winningDivId_'+i+'" style="margin-top: 20px;"><table id="purchasing_tab_pur_'+i+'" data-options="onClickRow: onClickRowPurRes"></table>'
...
function onClickRowPurRes(index,data) {
	$("#winningDivId").click(function(e){//在页面任意位置点击而触bai发此事件du
		var shortId = tabId;
		if(e.target.innerText==''){
			tabId =e.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[2].id;// e.target表示被点击的目标
			if(tabId==''){
				tabId =e.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.children[2].id;// e.target表示被点击的目标
			}
		}else{
