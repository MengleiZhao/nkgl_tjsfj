...
<form id="purchase_intention_check_form" method="post" data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
					...
					<div  title="意向公开申请" id="cgxxdiv" data-options="collapsed:false,collapsible:false"
						...
						<table cellpadding="0" cellspacing="0" class="ourtable">
							...
							<tr class="trbody">
								...
								<td class="td2">
									<input class="easyui-textbox" type="text" required="required" id="f_amount" name="amount" readonly="readonly" data-options="icons: [{iconCls:'icon-yuan'}],validType:'length[1,25]',formatter:function(value,row,index){return fomatMoney(value,2);}" style="width: 200px" value="${bean.amount}"/>
								</td>
							</tr>
							<tr class="trbody">
...
<script type="text/javascript">
	...
	function check(checkResult) {
	 	...
	 	$('#purchase_intention_check_form').form('submit', {
			onSubmit : function() {
				$.messager.progress();
			},
			url : base + '/purchaseIntentionCheck/checkResult',
			success : function(data) {
				$.messager.progress('close');
				data = eval("(" + data + ")");
				if (data.success) {
					$.messager.alert('系统提示', data.info, 'info');
					$('#purchase_intention_check_Tab').datagrid('reload');
