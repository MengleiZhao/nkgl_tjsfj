...
<div class="list-div">
	...
	<div class="list-top">
		<table id="check_payfor_dg"  class="top-table" cellpadding="0" cellspacing="0">
			<tr>
			<td class="top-table-search">采购批次编号&nbsp;
					<input id="check_payfor_fpCode" name="fpCode" style="width: 150px; height:25px;" class="easyui-textbox"></input>
					&nbsp;&nbsp;采购名称
					<input id="check_payfor_fpName" name="fpName" style="width: 150px; height:25px;" class="easyui-textbox"></input>
					&nbsp;&nbsp;申报部门
					<input id="check_payfor_fDeptName" name="fDeptName" style="width: 150px; height:25px;" class="easyui-textbox"></input>
	...
	<div style="margin: 0 10px 0 10px;height: 445px;">	
		<table id="check_payfor_receive_tab" class="easyui-datagrid"
			data-options="collapsible:true,url:'${base}/cgpayforcheck/cgpayforcheckPage',
		...
		selectOnCheck: true,checkOnSelect:true,remoteSort:true,nowrap:false,striped: true,fitColumns: true">
			<thead>
				<tr>
					...
					<th data-options="field:'fpCode',align:'left'" style="width: 12%">采购批次编号</th>
					<th data-options="field:'fpName',align:'left'" style="width: 12%">采购名称</th>
					<th
	...
	//分页样式调整
	$(function(){
		var pager = $('#check_payfor_receive_tab').datagrid().datagrid('getPager');	// get the pager of datagrid
		pager.pagination({
	...
	function retract() {
		...
		function queryPayfor() {
			...
			$('#check_payfor_receive_tab').datagrid('load', {
				fpCode:$('#check_payfor_fpCode').val(),
				fpName:$('#check_payfor_fpName').val(),
				fDeptName:$('#check_payfor_fDeptName').val()
			});
		}
		//清除查询条件
		function clearPayforTable() {
			...
			$("#check_payfor_fpCode").textbox('setValue','');
			$("#check_payfor_fpName").textbox('setValue','');
			$("#check_payfor_fDeptName").textbox('setValue','');
			$('#check_payfor_receive_tab').datagrid('load',{//清空以后，重新查一次
			});
		}
		//时间格式化
	...
	function CZ(val, row) {
		if (x == 9 ) {
			...
			return 	'<table><tr style="width: 75px;height:20px"><td style="width: 25px">'+
					'<a href="#" onclick="checkPayApplya(' + row.fpId + ')" class="easyui-linkbutton">'+
	...
	//跳转审批页面
	function checkPayApplya(id) {
		var win = creatWin(' ', 970, 580, 'icon-search', "/cgpayforcheck/check?id="+ id);
		win.window('open');
	}
	</script>
</body>
