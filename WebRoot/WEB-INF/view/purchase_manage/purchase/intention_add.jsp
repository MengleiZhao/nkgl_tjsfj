...
<form id="purchase_intention_form" method="post" data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
		<div class="window-left-div" style="width:760px;height: 491px;border: 1px solid #D9E3E7;margin-top: 20px;" data-options="region:'west',split:true">
						...
						<table cellpadding="0" cellspacing="0" class="ourtable">
							...
							<tr class="trbody">
								...
								<td class="td4">
									...
									<input type="hidden" name="stauts" id="stauts" value="${bean.stauts}"/><!--删除状态  -->
							...
							<tr class="trbody">
								...
								<td class="td2">
									<input class="easyui-textbox" type="text" required="required" name="fPurchaseName" data-options="validType:'length[1,25]'" style="width: 200px" value="${bean.fPurchaseName}"/>
								</td>
								<td class="td4"></td>
								<td class="td1"><span class="style1">*</span>&nbsp;预算金额</td>
								<td class="td2">
									<input class="easyui-textbox" type="text" required="required" id="f_amount" name="amount" data-options="icons: [{iconCls:'icon-yuan'}],validType:'length[1,25]'" style="width: 200px" value="${bean.amount}"/>
								</td>
							</tr>
							<tr class="trbody">
								<td class="td1"><span class="style1">*</span>&nbsp;预计采购时间</td>
								<td colspan="4">
									<input id="purchaseTime" name="fPurchaseTime" type="text" class="easyui-datebox" style="width: 200px" value="${bean.fPurchaseTime}" data-options="editable:false"/>
								</td>
							</tr>
							<tr style="height: 70px;">
								<td class="td1"><span class="style1">*</span>&nbsp;采购需求概况</td>
								<td colspan="4">
									<textarea class="textbox-text" id="fPurchaseDemand" name="fPurchaseDemand" oninput="textareaNum(this,'textareaNum1')" autocomplete="off"  style="width:555px;height:70px">${bean.fPurchaseDemand}</textarea>
								</td>
							</tr>
							<tr class="trbody">
								<td class="td1"><span class="style1">*</span>&nbsp;是否专门面向中小企业</td>
								<td class="td2" colspan="4">
         							<input type="radio" name="isForSmes" <c:if test="${bean.isForSmes=='0'}">checked="checked"</c:if> value="0">是</input>
         							<input type="radio" name="isForSmes" <c:if test="${bean.isForSmes!='0'}">checked="checked"</c:if> value="1">否</input>
								</td>
							</tr>
							<tr style="height: 70px;">
								<td class="td1">备注</td>
								<td colspan="4">
									<textarea class="textbox-text" id="remark" name="remark" oninput="textareaNum(this,'textareaNum1')" autocomplete="off"  style="width:555px;height:70px">${bean.remark}</textarea>
								</td>
							</tr>
							<tr class="trbody">
								<td class="td1">
			...
			<div class="window-left-bottom-div">
				<a href="javascript:void(0)" onclick="savePurchaseIntention(0)">
					<img src="${base}/resource-modality/${themenurl}/button/zhanchun1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">
				</a>&nbsp;&nbsp;
				<a href="javascript:void(0)" onclick="savePurchaseIntention(1)">
...
<script type="text/javascript">
	...
	//保存
	function savePurchaseIntention(fCheckStauts) {
		var f_amount = $('#f_amount').val();
		if(!isNaN(f_amount)){
			if(f_amount < 500000){
				$.messager.alert('系统提示', '预算金额不能小于50万', 'info');
				return;
			}
		}
		
		//设置审批状态
		$('#flowStauts').val(fCheckStauts);
		
		$('#stauts').val(fCheckStauts);
		//附件的路径地址
		var s="";
		...
		//提交
		$('#purchase_intention_form').form('submit', {
			onSubmit : function() {
				//获得校验结果
			...
			url : base + '/purchaseIntention/save',
