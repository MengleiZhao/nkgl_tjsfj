...
<div class="window-table" style="margin-bottom:10px;">
	...
	<table id="dg_reception_people_plan_detail" class="easyui-datagrid" style="width:707px;height:auto"
	data-options="
	singleSelect: true,
	<!-- toolbar: '#rpp', -->
...
function endEditingRPE() {
	...
	if ($('#dg_reception_people_plan_detail').datagrid('validateRow', editPeopleIndex)) {
		var rpp = $('#dg_reception_people_plan_detail').datagrid('getEditor', {
			index : editPeopleIndex,
			field : 'costDetail'
		});
		$('#dg_reception_people_plan_detail').datagrid('endEdit', editPeopleIndex);
...
function onClickRowRPE(index) {
	if (editPeopleIndex != index) {
		if (endEditingRPE()) {
			$('#dg_reception_people_plan_detail').datagrid('selectRow', index).datagrid('beginEdit',
					index);
			editPeopleIndex = index;
		} else {
			$('#dg_reception_people_plan_detail').datagrid('selectRow', editPeopleIndex);
		}
	}
}
function appendRPE() {
	if (endEditingRPE()) {
		$('#dg_reception_people_plan_detail').datagrid('appendRow', {});
		editPeopleIndex = $('#dg_reception_people_plan_detail').datagrid('getRows').length - 1;
		$('#dg_reception_people_plan_detail').datagrid('selectRow', editPeopleIndex).datagrid('beginEdit',editPeopleIndex);
	}
}
function removeitRPE() {
	...
	$('#dg_reception_people_plan_detail').datagrid('cancelEdit', editPeopleIndex).datagrid('deleteRow',
			editPeopleIndex);
	editPeopleIndex = undefined;
}
function acceptRPE() {
	if (endEditingRPE()) {
		$('#dg_reception_people_plan_detail').datagrid('acceptChanges');
	}
}
...
/* 	//重新计算开支标准
	function countkzbz() {
		accept();
		//获得接待人数（有几行就是接待多少人）
		var rownum = $('#dg_reception_people_plan_detail').datagrid('getRows').length;
		//修改明细表中的开支标准
