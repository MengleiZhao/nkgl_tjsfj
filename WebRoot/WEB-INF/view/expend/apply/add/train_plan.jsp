...
function savePlan(){
	...
	for (var i = rows.length-1; i >= 0; i--) {
		...
		if(rows[i].lecturerName != null && rows[i].lecturerName != "" && rows[i].lecturerName != undefined){
			if(rows[i].lessonTime == null || rows[i].lessonTime == "" || rows[i].lessonTime == undefined){
				alert("请填写学时！");
				return;
			}
		}
...
//接待人员表格添加删除，保存方法
	...
	function lessonTime(newValue,oldValue){
		...
		if(newValue == undefined){
			if(parseInt(tr[4].target.textbox('getValue'))>parseInt((endTime-beginTime))*4/3){
				alert("请填写正确的学时！");
				tr[4].target.textbox('setValue',0);
				return;
			}
		}else{
			if(parseInt(newValue)>parseInt((endTime-beginTime)*4/3)){
				alert("请填写正确的学时！");
				tr[4].target.textbox('setValue',0);
				return;
			}
