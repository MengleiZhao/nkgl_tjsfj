...
function endEditingRSP() {
	...
	if ($('#dg_reception_strok_plan_detail').datagrid('validateRow', editStrokIndex)) {
		var dmp = $('#dg_reception_strok_plan_detail').datagrid('getEditor', {
			index : editStrokIndex,
			field : 'fPro'
		});
		$('#dg_reception_strok_plan_detail').datagrid('endEdit', editStrokIndex);
...
function onClickRowRSP(index) {
	if (editStrokIndex != index) {
		if (endEditingRSP()) {
			$('#dg_reception_strok_plan_detail').datagrid('selectRow', index).datagrid('beginEdit',
					index);
			editStrokIndex = index;
		} else {
			$('#dg_reception_strok_plan_detail').datagrid('selectRow', editStrokIndex);
		}
	}
}
function appendRSP() {
	if (endEditingRSP()) {
		$('#dg_reception_strok_plan_detail').datagrid('appendRow', {});
		editStrokIndex = $('#dg_reception_strok_plan_detail').datagrid('getRows').length - 1;
		$('#dg_reception_strok_plan_detail').datagrid('selectRow', editStrokIndex).datagrid('beginEdit',editStrokIndex);
	}
}
function removeitRSP() {
	...
	$('#dg_reception_strok_plan_detail').datagrid('cancelEdit', editStrokIndex).datagrid('deleteRow',editStrokIndex);
	editStrokIndex = undefined;
}
function accept3() {
	if (endEditingRSP()) {
		$('#dg_reception_strok_plan_detail').datagrid('acceptChanges');
	}
}
...
/* 	//重新计算开支标准
	function countkzbz() {
		accept();
		//获得接待人数（有几行就是接待多少人）
		var rownum = $('#dg_reception_strok_plan_detail').datagrid('getRows').length;
		//修改明细表中的开支标准
	...
	/* function changeTime1(){
		var index=$('#dg_reception_strok_plan_detail').datagrid('getRowIndex',$('#dg_reception_strok_plan_detail').datagrid('getSelected'));
		var editors = $('#dg_reception_strok_plan_detail').datagrid('getEditors', index);  
