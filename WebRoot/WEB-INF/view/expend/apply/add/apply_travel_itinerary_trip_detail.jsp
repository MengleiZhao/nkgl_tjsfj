	...
	">
		<thead>
			<tr>
				...
				<th data-options="field:'travelAreaTime',width:130,align:'center',editor:{type:'datebox', options:{onChange:settravelAreaTime,editable:false}}">出行日期</th>
				<th data-options="field:'travelAreaName',width:158,align:'center',editor:{type:'textbox',options:{editable:false,icons:[{iconCls:'icon-add',handler: function(e){
									     var row = $('#tracel_itinerary_trip_tab_id').datagrid('getSelected');
									     var index = $('#tracel_itinerary_trip_tab_id').datagrid('getRowIndex',row);
									     selectRegionDetailTrip(index)
									     }}]}}">目的地</th>
				<th data-options="field:'travelAreaId',align:'center',hidden:true,editor:{type:'textbox',options:{editable:false}}"></th>
				<th data-options=" field:'areaNames',width:150,align:'center',
			                        editor:{
			                            type:'combobox',
			                            options:{
			                                valueField:'code',
				...
				<th data-options="field:'areaCode',hidden:true,align:'center',editor:{type:'textbox',options:{editable:false}}"></th>
				<th data-options=" field:'fDriveWay',width:150,align:'center',
			                        editor:{
			                            type:'combobox',
			                            options:{
			                                valueField:'code',
...
/* 	$('#tracel_itinerary_trip_tab_id').datagrid('reload');
	...
	$("#applyTotalAmountTrip").html(listToFixed((parseFloat(foodAmount)+parseFloat(hotelAmount)+parseFloat(cityAmount)+parseFloat(outsideAmount)))+"元");
