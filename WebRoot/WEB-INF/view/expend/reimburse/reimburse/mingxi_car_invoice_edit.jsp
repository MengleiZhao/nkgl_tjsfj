...
<form id="form1">
	<table id="carfapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<c:forEach items="${Invoicelist1 }" varStatus="i" var="invoice">
			<tbody id="car_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="carFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="carfp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="carImage_${i.index}" onchange="upImageFile(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="carZfbimagetd_${i.index}" style="width: 150px;">
						<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="deleteImage(this)" id="carsc_${i.index}">    删除</a>
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								...
								<input class="input_amonut1 a easyui-numberbox" name="amount" id="carAmount_${i.index}" value="${invoice.amount}" onchange="addAmonut(this)"data-options="precision:2"/>
								<!-- oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value=""  -->
							</p>
							<p style="margin-bottom:5px;" >
								<span class="fp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" name="remark" id="carRemark_${i.index}" value="${invoice.remark}" />
							</p>
						</div>
					</td>
...
function countDays(obj){
	...
	if (d > 0) {
		var amount =parseFloat($('#carAmount_' + i).val());
		if(!isNaN(amount)){
...
function countTravel(obj){
	...
	var i=id.substring(j+1,id.length);
	var num =0;
	var num1 =parseFloat($('#carAmount_' + i).val());
	...
	function append(obj) {
		...
		$('#carfapiao').append('<tbody id="car_'+a+'" >'+
		'<tr><td class="td1" style="width: 100px;">&nbsp;&nbsp;<span id="carFp_'+a+'">发票'+(fromNum+1)+'</span>'+
		'<input type="text" id="carfp_'+a+'" name="fileId" hidden="hidden">'+
		'<input type="file" id="carImage_'+a+'" onchange="upImageFile(this)" hidden="hidden"></td>'+
		'<td class="td2" colspan="3" id="carZfbimagetd_'+a+'" style="width: 150px;">'+
		...
		'<p style="margin-bottom:10px;">'+
		'<span class="fp_span" >金额(元)：</span>'+
		'<input class="input_amonut1 a easyui-numberbox" name="amount" id="carAmount_'+a+'" onchange="addAmonut(this)"data-options="precision:2"/>'+
		'</p>'+
		'<p style="margin-bottom:5px;" >'+
		'<span class="fp_span">备注：</span>'+
		'<input  class="input_amonut1 easyui-textbox" name="remark" id="carRemark_'+a+'" value="" />'+
		'</p></div>'+
		'</td>'+
		'<td style="vertical-align:top">'+
		'<div>'+
			...
			$.ajax({
						...
						success : function(data) {
				        	...
				        	if(y!=0){
				        		$('#carZfbimagetd_' + j)
								.append(
										'<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="carsc_'
												+ j
												+ '" onclick="deleteImage(this)">    删除</a>');
					        	}else{
					        		$('#carZfbimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="carsc_'+j+'" onclick="deleteImage(this)">    删除</a>');
					        	}
		...
		var id =obj.id;
	     ...
	     var y=num.substring(x+1,num.length);
			...
			.append(
      		...
      		$('#carZfbimagetd_'+j).append('<a onclick="clickUpFile(this)" id="click_'+j+'" style="font-weight: bold;" href="#">'+
	...
	function  deleteTravel(item){
		...
		for(var i=1;i<fromNum;i++){
			if('car'==arr[0]){
				var tableId = $('#carfapiao').find('tbody');
