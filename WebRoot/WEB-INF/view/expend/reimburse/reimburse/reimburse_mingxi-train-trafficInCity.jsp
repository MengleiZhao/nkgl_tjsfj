...
<div class="window-tab" style="margin-left: 0px;padding-top: 10px">
	...
	<div id="trafficInCity_Id" style="height:30px;padding-top : 8px">
		...
		<a style="float: left;color: #666666;font-size:12px;">申请金额：<span ><fmt:formatNumber groupingUsed="true" value="${trainingBean.transportMoney}" minFractionDigits="2" maxFractionDigits="2"/>[元]</span></a>
		<a style="float: left;">&nbsp;&nbsp;</a>
		<c:if test="${operation=='add'}">
		<a style="float: left;color: #666666;font-size:12px;">汇总金额：<span id="traffic2Amount">[元]</span></a>
		</c:if>
		<c:if test="${operation!='add'}">
		<a style="float: left;color: #666666;font-size:12px;">汇总金额：<span id="traffic2Amount"><fmt:formatNumber groupingUsed="true" value="${reimbTrainingBean.transportMoney}" minFractionDigits="2" maxFractionDigits="2"/>[元]</span></a>
		</c:if>
	</div>
	...
	<thead>
		<tr>
			<th data-options="field:'thId',hidden:true"></th>
				...
				<c:if test="${empty detail}">
				<th data-options="field:'reimbSum',required:'required',align:'center',width:220,editor:{type:'numberbox',options:{onChange:addNum6,precision:2,groupSeparator:','}}">报销金额[元]</th>
				</c:if>
				<c:if test="${!empty detail}">
				<th data-options="field:'reimbSum',required:'required',align:'center',width:220,editor:{type:'numberbox',options:{precision:2,groupSeparator:','}}">报销金额[元]</th>
				</c:if>
		</tr>
	</thead>
	</table>
