...
<div class="window-tab" style="margin-left: 0px;padding-top: 10px;border-top: 1px solid rgba(217,227,231,1);">
	...
	">
		<thead>
			<tr>
				...
				<th data-options="field:'fApplyAmount',width:227,align:'center',editor:{type:'numberbox',options:{editable:true,precision:2}}">补贴金额</th>
			</tr>
		</thead>
	</table>
	<div id="reimburse_in_city_id" style="height:30px;padding-top : 8px">
		...
		<a style="float: left;color: #666666;font-size:12px;">原申请金额：<span id=""><fmt:formatNumber groupingUsed="true" value="${applyBean.cityAmount}" minFractionDigits="2" maxFractionDigits="2"/>[元]</span></a>
		<a style="float: left;">&nbsp;&nbsp;</a>
...
function endEditingInCity() {
	...
	if ($('#reimbursein_city_tab_id').datagrid('validateRow', editIndexInCity)) {
		//下面三行，是在增加一行的时候，防止原来的一行的值变成code
		var tr = $('#reimbursein_city_tab_id').datagrid('getEditors', editIndexInCity);
		$('#reimbursein_city_tab_id').datagrid('endEdit', editIndexInCity);
...
function onClickRowInCity(index) {
	if (editIndexInCity != index) {
		if (endEditingInCity()) {
			$('#reimbursein_city_tab_id').datagrid('selectRow', index).datagrid('beginEdit',
					index);
			editIndexInCity = index;
		} else {
			$('#reimbursein_city_tab_id').datagrid('selectRow', editIndexInCity);
		}
	}
}
function appendInCity() {
	if (endEditingInCity()) {
		...
		editIndexInCity = $('#reimbursein_city_tab_id').datagrid('getRows').length - 1;
		$('#reimbursein_city_tab_id').datagrid('selectRow', editIndexInCity).datagrid('beginEdit',editIndexInCity);
	}
}
function removeitInCity() {
	...
	$('#reimbursein_city_tab_id').datagrid('cancelEdit', editIndexInCity).datagrid('deleteRow',
			editIndexInCity);
	editIndexInCity = undefined;
	var rows = $('#reimbursein_city_tab_id').datagrid('getRows');
	var travelDays=0;
	var hotelDays=0;
...
function acceptInCity() {
	if (endEditingInCity()) {
		$('#reimbursein_city_tab_id').datagrid('acceptChanges');
	}
}
...
function setDays5(newValue,oldValue) {
	var totalDays = 0;
	var fsumDays = 0;
	var index=$('#reimbursein_city_tab_id').datagrid('getRowIndex',$('#reimbursein_city_tab_id').datagrid('getSelected'));
	var rows = $('#reimbursein_city_tab_id').datagrid('getRows');
    var editors = $('#reimbursein_city_tab_id').datagrid('getEditors', index);  
    var day1 = editors[0]; 
    var day2 = editors[1]; 
...
function inCityJson(){
	acceptInCity();
	var rows2 = $('#reimbursein_city_tab_id').datagrid('getRows');
	var inCity = "";
	if(rows2==''){
	}else{
