...
<%@ include file="/includes/taglibs.jsp"%>
	...
	<tr class="trbody">
		...
		<td class="td2">
			...
			name="reDateStart" class="easyui-datebox" <c:if test="${bean.fupdateStatus!='1'}"> readonly="readonly"</c:if>
			data-options="required:true"
			value="${receptionBeanEdit.reDateStart}" editable="false"/>
		</td>
		<td class="td1" style="width: 70px">结束日期</td>
		<td class="td2">
			<input style="width: 230px; height: 30px;" id="reDateEnd" <c:if test="${bean.fupdateStatus!='1'}"> readonly="readonly"</c:if>
	...
	<tr class="trbody">
		...
		<td class="td2">
			<input type="radio" onclick="stayYNCheck1()" value="1" disabled="disabled" name="box" id="box1" <c:if test="${receptionBean.stayYN=='1'}">checked="checked" </c:if> />是
			<input type="radio" onclick="stayYNCheck2()" value="0" disabled="disabled" name="box" id="box2" <c:if test="${receptionBean.stayYN=='0'}">checked="checked" </c:if> />否
			<input type="hidden" id="reimstayYN" name="stayYN"  value="${receptionBean.stayYN}"/>
		</td>
	</tr>
	<tr class="trbody">
		...
		<td class="td2" >
		<input class="easyui-textbox" id="deptName" name="deptName" readonly="readonly" value="${bean.deptName}" style="width: 230px;height: 30px;margin-left: 10px " >
		</td>
	</tr>
	<tr style="height: 70px;">
		<td class="td1" style="width: 70px;"><span class="style1">*</span>接待内容</td>
		<td colspan="3">
			<textarea name="receptionContent" readonly="readonly" id="receptionContent" class="easyui-textbox" <c:if test="${bean.fupdateStatus!='1'}"> readonly="readonly"</c:if> oninput="textareaNum(this,'textareaNum2')" autocomplete="off" 
...
function stayYNCheck1(){
	$('#reimstayYN').val(1);
   	...
   	$.parser.parse('#rec-hotel-div-reim');
}
