...
<form id="formCostRent">
		...
		<tbody id="costRent_0" >
			<tr>
				<td class="td1" style="width: 100px;">
					&nbsp;&nbsp;<span id="costRentFp_0">发票1</span>
					<input type="text" id="costRentfp_0" name="fileId" hidden="hidden">
					<input type="file" id="costRentImage_0" accept="image/jpeg,image/jpg,image/png" onchange="upImageFileCostRent(this)" hidden="hidden">
				</td>
				<td class="td2" colspan="3" id="costRentZfbimagetd_0" style="width: 150px;">
					<a onclick="clickUpFileCostRent(this)" id="costRentClick_0" style="font-weight: bold;" href="#">
				...
				<td class="td2" colspan="3" id="">
					<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
						<p style="margin-bottom:10px;">
							<span class="costRentfp_span" >金额(元)：</span>
							<input class="input_amonut1 a easyui-numberbox" name="amount" id="costRentAmount_0" onchange="addAmonut(this)"data-options="precision:2"/>
						</p>
						<p style="margin-bottom:5px;" >
							<span class="costRentfp_span">备注：</span>
							<input  class="input_amonut1 easyui-textbox" name="remark" id="costRentRemark_0" value="" />
						</p>
					</div>
				</td>
			</tr>
	...
	<a onclick="appendCostRent(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function countDays(obj){
	...
	if (d > 0) {
		var amount =parseFloat($('#costRentAmount_' + i).val());
		if(!isNaN(amount)){
...
function countTravel(obj){
	...
	var i=id.substring(j+1,id.length);
	var num =0;
	var num1 =parseFloat($('#costRentAmount_' + i).val());
	...
	function appendCostRent(obj) {
		...
		'<a onclick="clickUpFileCostRent(this)" id="costRentClick_'+a+'" style="font-weight: bold;" href="#">'+
		'<img style="vertical-align:bottom;margin-left:30px" src="${base}/resource-modality/${themenurl}/button/shangchuan1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">'+
		'</a></td>'+
		'<td class="td2" colspan="3" id="">'+
		'<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">'+
		'<p style="margin-bottom:10px;">'+
		'<span class="costRentfp_span" >金额(元)：</span>'+
		'<input class="input_amonut1 a easyui-numberbox" name="amount" id="costRentAmount_'+a+'" onchange="addAmonut(this)"data-options="precision:2"/>'+
		'</p>'+
		'<p style="margin-bottom:5px;" >'+
		'<span class="costRentfp_span">备注：</span>'+
		'<input  class="input_amonut1 easyui-textbox" name="remark" id="costRentRemark_'+a+'" value="" />'+
		'</p></div>'+
		'</td>'+
						...
						success : function(data) {
				        	...
				        	if(y!=0){
								...
								.append(
					        		...
					        		$('#costRentZfbimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="sc_'+j+'" onclick="deleteImageCostRent(this)">    删除</a>');
					        	}
		...
		var id =obj.id;
	     ...
	     var y=num.substring(x+1,num.length);
	   	  if(y!=0){
	   		$('#costRentZfbimagetd_' + j)
			.append(
					'<a onclick="clickUpFileCostRent(this)" id="costRentClick_'
							+ j
      		...
      		$('#costRentZfbimagetd_'+j).append('<a onclick="clickUpFileCostRent(this)" id="costRentClick_'+j+'" style="font-weight: bold;" href="#">'+
