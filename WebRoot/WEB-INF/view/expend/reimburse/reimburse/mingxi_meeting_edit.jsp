	...
	<table id="tableHotel" class="window-table-readonly" cellspacing="0" cellpadding="0" style="">
			...
			<tr>
				...
				<td class="td2" colspan="4" id="zfbimagetd_${st1.index}">
					...
					<c:if test="${fp1.fileId!=null&&!empty fp1.fileId}"> 
					<img style="vertical-align:bottom;margin-left:30px;width: 100px; height: 73px;" src="${base}/attachment/download/${fp1.fileId}" onclick="yl(this.src)"/><a style="color:red" href="#" onclick="deleteImage(this)" id="sc_${st1.index}">    删除</a>
					</c:if>
					<c:if test="${st1.index !=0}">
					<a onclick="deleteMeetings(this)" style="float: right;" href="#">
...
<form id="form3">
	<table id="tableOther" class="window-table-readonly" cellspacing="0" cellpadding="0" style="">
		...
		<c:forEach items="${fp3}" var="fp3" varStatus="st3">		
			...
			<tr>
				<td class="td1">
					&nbsp;&nbsp;<span id="otherFp_${st3.index}">发票${st3.index+1}</span>
...
function append(obj){
		...
		$('#' +fromNum[fromNum.length-1].id).after(addItem);
