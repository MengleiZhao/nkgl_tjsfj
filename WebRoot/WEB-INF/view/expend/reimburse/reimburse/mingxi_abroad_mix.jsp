		...
		<tbody id="mix_0" >
			<tr>
				<td class="td1" style="width: 100px;">
					&nbsp;&nbsp;<span id="mixFp_0">发票1</span>
					<input type="text" id="mixfp_0" name="fileId" hidden="hidden">
					<input type="file" id="mixImage_0" accept="image/jpeg,image/jpg,image/png" onchange="upImageFilemix(this)" hidden="hidden">
				</td>
				<td class="td2" colspan="3" id="mixZfbimagetd_0" style="width: 150px;">
					<a onclick="clickUpFilemix(this)" id="mixClick_0" style="font-weight: bold;" href="#">
				...
				<td class="td2" colspan="3" id="">
					<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
						<p style="margin-bottom:10px;">
							<span class="mixfp_span" >金额(元)：</span>
							<input class="input_amonut1 a easyui-numberbox" name="amount" id="mixAmount_0" onchange="addAmonut(this)"data-options="precision:2"/>
						</p>
						<p style="margin-bottom:5px;" >
							<span class="mixfp_span">备注：</span>
							<input  class="input_amonut1 easyui-textbox" name="remark" id="mixRemark_0" value="" />
						</p>
					</div>
				</td>
			</tr>
...
function countDays(obj){
	...
	if (d > 0) {
		var amount =parseFloat($('#mixAmount_' + i).val());
		if(!isNaN(amount)){
...
function countTravel(obj){
	...
	var i=id.substring(j+1,id.length);
	var num =0;
	var num1 =parseFloat($('#mixAmount_' + i).val());
	...
	function appendmix(obj) {
		...
		'<a onclick="clickUpFilemix(this)" id="mixClick_'+a+'" style="font-weight: bold;" href="#">'+
		'<img style="vertical-align:bottom;margin-left:30px" src="${base}/resource-modality/${themenurl}/button/shangchuan1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">'+
		'</a></td>'+
		'<td class="td2" colspan="3" id="">'+
		'<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">'+
		'<p style="margin-bottom:10px;">'+
		'<span class="mixfp_span" >金额(元)：</span>'+
		'<input class="input_amonut1 a easyui-numberbox" name="amount" id="mixAmount_'+a+'" onchange="addAmonut(this)"data-options="precision:2"/>'+
		'</p>'+
		'<p style="margin-bottom:5px;" >'+
		'<span class="mixfp_span">备注：</span>'+
		'<input  class="input_amonut1 easyui-textbox" name="remark" id="mixRemark_'+a+'" value="" />'+
		'</p></div>'+
		'</td>'+
						...
						success : function(data) {
				        	...
				        	if(y!=0){
								...
								.append(
					        		...
					        		$('#mixZfbimagetd_'+j).append('<img style="vertical-align:bottom;margin-left:0px; width: 100px;height: 73px" src="'+data+'" onclick="yl(this.src)"/><a style="color:red" href="#" id="sc_'+j+'" onclick="deleteImagemix(this)">    删除</a>');
					        	}
		...
		var id =obj.id;
	     ...
	     var y=num.substring(x+1,num.length);
	   	  if(y!=0){
	   		$('#mixZfbimagetd_' + j)
			.append(
					'<a onclick="clickUpFilemix(this)" id="mixClick_'
							+ j
      		...
      		$('#mixZfbimagetd_'+j).append('<a onclick="clickUpFilemix(this)" id="mixClick_'+j+'" style="font-weight: bold;" href="#">'+
