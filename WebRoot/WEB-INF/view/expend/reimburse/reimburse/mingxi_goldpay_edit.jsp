...
<form id="form1">
	<table id="goldPayfapiao" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style=" height: 100px;width:695px;">
		<c:forEach items="${Invoicelist }" varStatus="i" var="invoice">
			<tbody id="goldPay_${i.index}" >
				<tr>
					<td class="td1" style="width: 100px;">
						&nbsp;&nbsp;<span id="goldPayFp_${i.index}">发票${i.index+1}</span>
						<input type="text" id="goldPayfp_${i.index}" value="${invoice.fileId}" name="fileId" hidden="hidden">
						<input type="file" id="goldPayImage_${i.index}" accept="image/jpeg,image/jpg,image/png" onchange="goldPayUpImageFile(this)" hidden="hidden">
					</td>
					<td class="td2" colspan="3" id="goldPayimagetd_${i.index}" style="width: 150px;">
						<c:if test="${invoice.fileId!=null&&!empty invoice.fileId}">
							<img style="vertical-align:bottom;width: 100px; height: 73px;margin-left:0px" onclick="yl(this.src)" src="${base}/attachment/download/${invoice.fileId}"/><a style="color:red" href="#" onclick="goldPaydeleteImage(this)" id="goldPaysc_${i.index}">    删除</a>
						</c:if>
						<c:if test="${empty invoice.fileId}">
							<a onclick="goldPayClickUpFile(this)" id="goldPayClick_${i.index}" style="font-weight: bold;" href="#">
					...
					<td class="td2" colspan="3" id="">
						<div style="width: auto; height:auto; height:auto;margin-top:5px;margin-left:49px;width: 300px;">
							<p style="margin-bottom:10px;">
								...
								<input class="input_amonut1 a easyui-numberbox" name="amount" id="goldPayAmount_${i.index}" value="${invoice.amount }" onchange="goldPayaddAmonut(this)"data-options="precision:2"/>
							</p>
							<p style="margin-bottom:5px;" >
								<span class="goldPayfp_span">备注：</span>
								<input  class="input_amonut1 easyui-textbox" name="remark" id="goldPayRemark_${i.index}" value="${invoice.remark }" />
							</p>
						</div>
					</td>
					<td style="vertical-align:top">
						<div>
							<c:if test="${i.index!=0}">
								<a onclick="goldPayDeteleEdit(this)" style="float: right;" href="#" > 
	...
	<a onclick="goldPayappendEdit(this)" style="font-weight: bold;vertical-align: middle;margin-left: 308px;" href="#">
...
function goldPayappendEdit(obj) {
	index = index + 1;
	...
	'<tr><td class="td1" style="width: 100px;">&nbsp;&nbsp;<span id="goldPayFp_'+a+'">发票'+(fromNum+1)+'</span>'+
...
function  goldPayDeteleEdit(item){
