...
<form id="form1">
	<table id="tableHotel" class="window-table-readonly-zc" cellspacing="0" cellpadding="0" style="">
			<tr>
			...
			<tr>
				...
				<td class="td2" colspan="4" id="zfbimagetd_${st1.index}">
					...
					<a onclick="deleteTravels(this)" style="float: right;" href="#">
					...
					<div style="width: auto; height:auto;margin-top:5px;margin-left:49px;">
						...
						<p style="margin-bottom:5px;">
							...
							<span style="width: 80px;display: inline-block;" id="checkTrue_${st1.index}">
								<c:if test="${fp1.fileId!=null&&!empty fp1.fileId}">
									<img style="vertical-align:bottom;margin-left: 0px;" src="${base}/resource-modality/${themenurl}/left/zp.png">
								</c:if>
								<c:if test="${fp1.fileId==null||empty fp1.fileId}">
			...
			<tr>
				<td class="td2" colspan="5" >
					<div style="width: auto; height:auto;margin-top:5px;margin-left:49px;"">
						<p style="margin-bottom:5px;">
							<span class="fp_span">票面金额(元)：</span>
							<input  class="input_amonut a" name="amount"  id="amount_${st3.index+a+b}" onchange="countTravel(this)"
							oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value="${fp3.amount}" />
							<span class="fp_span1">退改等附加金额(元)：</span>
							<input  class="input_amonut a" name="amount1" id="amount1_${st3.index+a+b}" onchange="countTravel(this)"
							oninput = "value=value.replace(/^\D*(\d*(?:\.\d{0,2})?).*$/g, '$1')" value="${fp3.amount1}" />
						</p>
						<p style="margin-bottom:5px;">
							<span class="fp_span">金额小计(元)：</span>
							<input  class="input_amonut" name="amount2"  id="amount2_${st3.index+a+b}" 
							value="${fp3.amount2}" />
						</p>
						<p style="margin-bottom:5px;">
						...
						<p style="margin-bottom:5px;">
							...
							<input  class="input_amonut" name="vehicleLevel"  id="vehicleLevel_${st3.index+a+b}" type="text"
				...
				<td class="td2" colspan="5" >
					<div style="width: auto; height:auto;margin-top:5px;margin-left:49px;">
						<p style="margin-bottom:5px;">
							...
							<input  class="input_amonut" name="unit" id="unit_${st4.index+a+b+c}" 
							value="${fp4.unit}" />
						</p>
						<p style="margin-bottom:5px;">
						<span class="fp_span">交通工具：</span>
							<input  class="input_amonut" name="vehicle" id="vehicle_${st4.index+a+b+c}" type="text"
							value="${fp4.vehicle}" />
							<span class="fp_span1">出行日期：</span>
							<input  class="input_amonut" name="rideTime" id="rideTime_${st4.index+a+b+c}" type="date"
							value="${fn:substring(fp4.rideTime, 0, 10)}" />
						</p>
						<p style="margin-bottom:5px;">
							<span class="fp_span">出发地：</span>
							<input  class="input_amonut" name="placeStart" id="placeStart_${st4.index+a+b+c}"  type="text"
							value="${fp4.placeStart}" />
							<span class="fp_span1">到达地：</span>
							<input  class="input_amonut" name="placeEnd" id="placeEnd_${st4.index+a+b+c}" 
							value="${fp4.placeEnd}" />
						</p>
						<p style="margin-bottom:5px;">
							<span class="fp_span">出行人员：</span>
							<input  class="input_amonut" name="people" id="people_${st4.index+a+b+c}" 
							value="${fp4.people}" />
						</p>
						<p style="margin-bottom:5px;">
							<span class="fp_span">备注：</span>
...
function countTravel(obj){
	...
	function append3(obj) {
		...
		var amount1 = addItem.find('#amount1_'+z); //根据id查找子元素
		var amount2 = addItem.find('#amount2_'+z);
		var rideTime = addItem.find('#rideTime_'+z);
		var vehicle = addItem.find('#vehicle_'+z);
		var vehicleLevel = addItem.find('#vehicleLevel_'+z);
	...
	function append4(obj) {
		...
		var unit = addItem.find('#unit_'+m);
		var vehicle = addItem.find('#vehicle_'+m);
		var rideTime = addItem.find('#rideTime_'+m);
		var placeStart = addItem.find('#placeStart_'+m);
		var placeEnd = addItem.find('#placeEnd_'+m);
		var people = addItem.find('#people_'+m);
			...
			$.ajax({
						...
						success : function(data) {
				        	...
				        	if(y!=0){
				        		$('#zfbimagetd_' + j)
								.append(
										'<img style="vertical-align:bottom;margin-left:30px; width: 100px;height: 73px" src="'+data+'"/><a style="color:red" href="#" id="sc_'
												+ j
