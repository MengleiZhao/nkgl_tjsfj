...
<div  data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
	<div class="window-div">
		<div class="window-left-div" style="width:755px;height: 491px;border: 1px solid #D9E3E7;margin-top: 20px;">
			...
			<input id="payeeAmount" hidden="hidden"  />
					...
					<table class="window-table" style="margin-top: 3px;width: 695px" cellspacing="0" cellpadding="0">
						<tr class="trbody">
							...
							<td colspan="3" class="td2" >
								<input style="width: 635px; height: 30px;margin-left: 10px" name="summary"  id="summary" class="easyui-textbox"
								value="${bean.summary}" />
							</td>
						</tr>
						<c:if test="${detail=='2' }">
						...
						<tr class="trbody">
							...
							<td class="td2" >
								<input class="easyui-combobox" id="dirType" name="dirType" value="${bean.dirType}" style="width: 265px;height: 30px;margin-left: 10px "
								 data-options="
								 panelHeight:'auto',
							...
							<td class="td1" style="width: 70px;"><div id="spMatterName"><span class="style1">*</span>特殊事项</div></td>
							<td class="td2">
							<div id="spMatterVal">
							<input type="radio" value="1" onclick="radioyesDir()" name="spMatter" id="box1" <c:if test="${bean.spMatter=='1'}">checked="checked" </c:if> style="vertical-align: text-top;"/>&nbsp;&nbsp;是
							&nbsp;&nbsp;
							<input type="radio" value="0" onclick="radionoDir()" name="spMatter" id="box2" <c:if test="${bean.spMatter=='0' || empty bean.spMatter}">checked="checked" </c:if> style="vertical-align: text-top;"/>&nbsp;&nbsp;否
							</div>
							</td>
						</tr>
						<tr class="trbody" style="line-height: 60px">
							...
							<td colspan="3" class="td2" >
								<input style="width: 635px; height: 60px;margin-left: 10px;margin-top: 5px;" data-options="multiline:true" required="required" name="reason"  id="reason" class="easyui-textbox" value="${bean.reason}" />
							</td>
						</tr>
						<tr class="trbody">
						...
						<tr class="trbody"  style="display: none;">
...
function saveReimburse(flowStauts) {
	...
	//获取收款信息中的转账金额
	var payerMoney=isNaN(parseFloat($('#payeeAmount').val()))?0:parseFloat($('#payeeAmount').val());//转账金额
	var dirType = $("#dirType").combobox('getValue');
	if("ZJBXLX-2"==dirType){
		if(payerMoney>=10000){
			alert('报销金额大于等于10000元的，请走通用事项申请！');
			return false;
		}
	}
...
function radioyesDir(){
	
}

function onSelectDirType(rec){
	if("ZJBXLX-0"==rec.code){
		$("#spMatterName").html('');
		$("#spMatterVal").html('');
		$("#spMatterName").html('<span class="style1">*</span>特殊事项');
		$("#spMatterVal").html('<input type="radio" value="1" onclick="radioyesDir()" name="spMatter" id="box1" <c:if test="${bean.spMatter=='1'}">checked="checked" </c:if> style="vertical-align: text-top;"/>&nbsp;&nbsp;是&nbsp;&nbsp;<input type="radio" value="0" onclick="radionoDir()" name="spMatter" id="box2" <c:if test="${bean.spMatter=='0' || empty bean.spMatter}">checked="checked" </c:if> style="vertical-align: text-top;"/>&nbsp;&nbsp;否');
	}
	if("ZJBXLX-1"==rec.code){
		$("#spMatterName").html('');
		$("#spMatterVal").html('');
		$("#spMatterName").html('<span class="style1">*</span>特殊事项');
		$("#spMatterVal").html('<input type="radio" value="1" onclick="radioyesDir()" name="spMatter" id="box1" <c:if test="${bean.spMatter=='1'}">checked="checked" </c:if> style="vertical-align: text-top;"/>&nbsp;&nbsp;是&nbsp;&nbsp;<input type="radio" value="0" onclick="radionoDir()" name="spMatter" id="box2" <c:if test="${bean.spMatter=='0' || empty bean.spMatter}">checked="checked" </c:if> style="vertical-align: text-top;"/>&nbsp;&nbsp;否');
	}
	if("ZJBXLX-2"==rec.code){
		$("#spMatterName").html('');
		$("#spMatterVal").html('');
	}
}
