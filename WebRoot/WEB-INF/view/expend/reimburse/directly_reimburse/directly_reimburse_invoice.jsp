...
<table id="drfpdg" class="easyui-datagrid"
style="width:660px;height:auto"
data-options="
toolbar: '#drfptb',
<c:if test="${!empty bean.drId}">
url: '${base}/directlyReimburse/invoice?id=${bean.drId}',
</c:if>
<c:if test="${empty bean.drId}">
url: '',
</c:if>
...
">
	<thead>
		<tr>
			...
			<th data-options="field:'invoiceTime',align:'center',width:110,formatter:ChangeDateFormat">开票时间</th>
...
<c:if test="${empty detail}">
	<div id="drfptb" style="height:30px">
		<a href="javascript:void(0)" onclick="removeit2()" style="float: right;">
			<img src="${base}/resource-modality/${themenurl}/button/scyh1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">
		</a>
		<a style="float: right;">&nbsp;&nbsp;</a>
		<a href="javascript:void(0)" onclick="openInvoiceKind(${bean.drId})" style="float: right;">
			<img src="${base}/resource-modality/${themenurl}/button/tjfp1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">
		</a>
	</div>
</c:if>
<input type="hidden" id="drfpJson" name="fapiao"/>
		...
		{field: 'fpFileUpload', title: '发票附件上传', width: 185, 
			formatter:function (value, row, index) {
				if(row.fileId != "" && row.fileId != null) {
                	...
                	if(${empty detail}){
	                    var str = "#' style='color:blue' onclick='this.parentNode.childNodes[1].click()'>上传文件</a>"+
...
function openInvoiceKind(id) {
	var win = creatFirstWin('发票类型', 700, 400, 'icon-search', '/invoice/invoiceKind');
	win.window('open');
}
