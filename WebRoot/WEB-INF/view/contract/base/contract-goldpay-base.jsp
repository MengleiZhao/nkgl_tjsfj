						...
						<div  data-options="iconCls:'icon-xxlb',collapsed:false,collapsible:false" style="overflow:auto;margin-top:10px;">
							<table cellpadding="0" cellspacing="0"  class="ourtable">
								<tr class="trbody">
									...
									<td>
										<input class="easyui-combobox" id="goldpay_fWarType" required="required" name="fWarType" style="width: 200px" data-options="editable:false,panelHeight:'auto',url:'${base}/Formulation/lookupsJson?parentCode=HTBZJLX&selected=${goldPay.fWarType}',method:'POST',valueField:'code',textField:'text',editable:false,validType:'selectValid'">
									</td>
								</tr>
								<tr class="trbody" style="height: 110px">
									<td class="td1"><span class="style1">*</span>&nbsp;付款说明</td>
									<td colspan="4" >
										<%-- <input  class="easyui-textbox" required="required" id="Dis_fPayRemark" name="fPayRemark" data-options="validType:'length[1,200]',multiline:true" style="width: 555px;height:100px;" value="${goldPay.fPayRemark}"/> --%>
										<textarea name="fPayRemark"  id="Dis_fPayRemark"  class="textbox-text" oninput="textareaNum(this,'textareaNum${textClassNum }')" autocomplete="off"  style="width:750px;height:300px;resize:none">${bean.fPayRemark }</textarea>
									</td>
								</tr>
								<tr>
									<td align="right" colspan="5" style="padding-right: 0px;">
									可输入剩余数：<span id="textareaNum${textClassNum }" class="200">
										<c:if test="${empty goldPay.fPayRemark}">200</c:if>
										<c:if test="${!empty goldPay.fPayRemark}">${200-goldPay.fPayRemark.length()}</c:if>
									</span>
									</td>
								</tr>
								<!-- <tr class="trbody">
