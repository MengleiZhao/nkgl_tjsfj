						...
						<div title="" data-options="iconCls:'icon-xxlb',collapsed:false,collapsible:false" style="overflow:auto;margin-top:10px;">
							...
							<table cellpadding="0" cellspacing="0"  class="ourtable">
								...
								<tr class="trbody" style="height: 110px;" >
									...
									<td colspan="4">
										<%-- <input class="easyui-textbox" required="required" type="text" id="end_fEndRemark" name="fEndRemark" data-options="validType:'length[1,100]',multiline:true"   style="width:555px;height:100px" value="${end.fEndRemark}"/> --%>
										<textarea name="fEndRemark"  id="end_fEndRemark"  class="textbox-text" oninput="textareaNum(this,'textareaNum1')" autocomplete="off"  style="width:555px;height:100px">${end.fEndRemark }</textarea>
									</td>
								</tr>
								<tr style="height: 20px;">
									<td align="right" colspan="5" style="padding-right: 0px;color: #999999">
									可输入剩余数：<span id="textareaNum1" class="200">
										<c:if test="${empty end.fEndRemark}">200</c:if>
										<c:if test="${!empty end.fEndRemark}">${200-end.fEndRemark.length()}</c:if>
										</span>
									</td>
								</tr>
								<tr class="trbody">
									...
									<td>
										<input class="easyui-combobox" required="required" id="end_fEndType" name="fEndType" style="width: 200px" data-options="editable:false,panelHeight:'auto',url:'${base}/Formulation/lookupsJson?parentCode=HTZZ&selected=${end.fEndType}',method:'POST',valueField:'code',textField:'text',editable:false,validType:'selectValid'">
									</td>
								</tr>
								<tr class="trbody">
									<td class="td1">
