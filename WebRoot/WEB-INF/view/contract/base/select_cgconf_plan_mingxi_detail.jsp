...
<table id="contract_cgplan_dg_detail" class="easyui-datagrid" style="width:700px;height:auto"
data-options="
singleSelect: true,
toolbar: '#cgplantb',
rownumbers : true,
...
<thead>
	<tr>
		...
		<th data-options="field:'fsignPrice',align:'center',editor:{type:'numberbox',options:{precision:2}}" style="width: 17.5%">单价(元)</th>
		...
		<th data-options="field:'famount',align:'center',editor:{type:'numberbox',options:{precision:2,readonly:false,onChange:totalFsumMoney}}" style="width: 17.5%">中标金额(元)</th> -->
	</tr>
</thead>
</table>
...
function endEditing() {
	...
	if ($('#contract_cgplan_dg_detail').datagrid('validateRow', editIndex)) {
		var ed = $('#contract_cgplan_dg_detail').datagrid('getEditor', {
			index : editIndex,
			field : 'costDetail'
		});
		/* console.log(ed);
		...
		$('#contract_cgplan_dg_detail').datagrid('endEdit', editIndex);
		editIndex = undefined;
		return true;
	} else {
		return false;
...
function getcgConfPlan(){
	acceptContract();
	var rows = $('#contract_cgplan_dg_detail').datagrid('getRows');
	var entities= '';
...
function onClickRow(index) {
	if (editIndex != index) {
		if (endEditing()) {
			$('#contract_cgplan_dg_detail').datagrid('selectRow', index).datagrid('beginEdit', index);
			editIndex = index;
			if(fpItemsName==0){
				var ed = $('#contract_cgplan_dg_detail').datagrid('getEditor', {index:editIndex,field:'fsumMoney'});
				$(ed.target).numberbox('readonly',false); 		
			}
		} else {
			$('#contract_cgplan_dg_detail').datagrid('selectRow', editIndex);
		}
	}
}
function append() {//未配置采购类型不可添加采购清单
	 if (endEditing()) {
			$('#contract_cgplan_dg_detail').datagrid('appendRow', {});
			editIndex = $('#contract_cgplan_dg_detail').datagrid('getRows').length - 1;
			$('#contract_cgplan_dg_detail').datagrid('selectRow', editIndex).datagrid('beginEdit',editIndex);
		} 
...
function removeit() {
	...
	$('#contract_cgplan_dg_detail').datagrid('cancelEdit', editIndex).datagrid('deleteRow',editIndex);
	editIndex = undefined;
	//修改申请总额
	setFsumMoney(0,0);
}
...
function acceptContract() {
	if (endEditing()) {
		$('#contract_cgplan_dg_detail').datagrid('acceptChanges');
	}
}
...
function setFsumMoney(newValue,oldValue) {
	...
	var fsumMoney = 0;
	var index=$('#contract_cgplan_dg_detail').datagrid('getRowIndex',$('#contract_cgplan_dg_detail').datagrid('getSelected'));
	var rows = $('#contract_cgplan_dg_detail').datagrid('getRows');
	for(var i=0;i<rows.length;i++){
		if(i==index){
...
function setEditing(rows,index){
    var editors = $('#contract_cgplan_dg_detail').datagrid('getEditors', index);  
    var fnum = editors[1]; 
...
function totalFsumMoney(newValue,oldValue){
	if(fpItemsName==0){
		...
		for(var i=0;i<rows.length;i++){
			if(i==index){
				var editors = $('#contract_cgplan_dg_detail').datagrid('getEditors', i);
