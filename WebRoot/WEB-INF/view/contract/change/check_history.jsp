...
<!-- <div class="window-tab" style="margin-left: 0px">这个是去掉左移 -->
	...
	<table id="check-history-dg-onchange" class="easyui-datagrid"  style="width:695px;height:auto"
	data-options="
	<c:if test="${empty foCodeOnchange}">
	url: '',
	</c:if>
	<c:if test="${!empty foCodeOnchange}">
	url: '${base}/wflow/history?fpId=${fpIdOnchange}&foCode=${foCodeOnchange}',
	</c:if>
	method: 'post'
	">
	<thead>
		<tr>
			...
			<th data-options="field:'fcheckTime',required:'required',align:'center',width:'27.5%',formatter: ChangeDateFormatIndex">审批时间</th>
			<th data-options="field:'fcheckRemake',required:'required',align:'left',width:'26%',formatter:formatCellTooltip">审批意见</th>
			<th data-options="field:'filesPid',align:'center',resizable:false,sortable:true,formatter:filesshow1" width="20%">审批附件</th>
		</tr>
	</thead>
	</table>
</div>
...
function filesshow1 (val,row){
	if (val=="" || val==null) {
		...
		var data = val.split(',');
		if(data.length>0){
			return 	'<table><tr style="width: 75px;height:20px"><td><a href="#" onclick="accessory1(\''+val+'\')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/fj1.png">' +
					'</a></td></table>';
		}else {
