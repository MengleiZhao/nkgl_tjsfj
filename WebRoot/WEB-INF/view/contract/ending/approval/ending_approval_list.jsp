...
function mouseOut(img) {
	...
	function ending_add() {
		...
		//先打开合同页面，让用户选择
		var win = creatFirstWin('合同拟定申请', 1115, 600, 'icon-search', '/ending/endingContract');
		win.window('open');
	}
	function ending_aprroval_detail(id) {
			var win = creatWin('合同拟定申请', 1115, 600, 'icon-search',"/ending/detail/" + id);
			win.window('open');
	}
	/* function editCF() {
	...
	function ending_approval(id) {
		//var row = $('#ending_approval_dg').datagrid('getSelected');
		var win = creatWin('合同终止审批', 1115, 600, 'icon-search',"/ending/approval/" + id);
		win.window('open');
	}
