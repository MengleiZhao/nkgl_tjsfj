<%@ page language="java" pageEncoding="UTF-8"%>
   <table id="fixed_add_plan3" class="easyui-datagrid"  style="height:auto;"
		data-options="
			singleSelect: true,
			rownumbers:true,
	...
	<thead>
		<tr>
			...
			<th data-options="field:'fDepreciationStatusShow',align:'center'" style="width:20%">折旧状态</th>
...
function endEditingPlan3() {
	...
	if ($('#fixed_add_plan3').datagrid('validateRow', editIndexPlan3)) {
		//下面三行，是在增加一行的时候，防止原来的一行的值变成code
		/* var tr = $('#fixed_add_plan3').datagrid('getEditors', editIndexPlan3);
		...
		$('#fixed_add_plan3').datagrid('endEdit', editIndexPlan3);
...
function onClickRowPlan3(index) {
	if (editIndexPlan3 != index) {
		if (endEditingPlan3()) {
			$('#fixed_add_plan3').datagrid('selectRow', index).datagrid('beginEdit',index);
			editIndexPlan3 = index;
		} else {
			$('#fixed_add_plan3').datagrid('selectRow', editIndexPlan3);
		}
	}
}
function appendPlan3() {
	if (endEditingPlan3()) {
		$('#fixed_add_plan3').datagrid('appendRow', {
		});
		editIndexPlan3 = $('#fixed_add_plan3').datagrid('getRows').length - 1;
		$('#fixed_add_plan3').datagrid('selectRow', editIndexPlan3).datagrid('beginEdit',editIndexPlan3);
	}
}
function removeitPlan3() {
	...
	$('#fixed_add_plan3').datagrid('cancelEdit', editIndexPlan3).datagrid('deleteRow',
			editIndexPlan3);
	editIndexPlan3 = undefined;
}
function acceptPlan3() {
	if (endEditingPlan3()) {
		$('#fixed_add_plan3').datagrid('acceptChanges');
	}
}
function getPlan3() {
	acceptPlan3();
	var rows = $('#fixed_add_plan3').datagrid('getRows');
	var entities = '';
...
function reloadDepreciationCode(){
	var index=$('#fixed_add_plan3').datagrid('getRowIndex',$('#fixed_add_plan3').datagrid('getSelected'));
	var fDepreciationStatusCode = $('#fixed_add_plan3').datagrid('getEditor',{
		index:index,
		field:'fDepreciationStatusCode'
	});
	var fDepreciationStatusShow = $('#fixed_add_plan3').datagrid('getEditor',{
		index:index,
...
function reloadValueCode(){
	var index=$('#fixed_add_plan3').datagrid('getRowIndex',$('#fixed_add_plan3').datagrid('getSelected'));
	var fValueTypeCode = $('#fixed_add_plan3').datagrid('getEditor',{
		index:index,
		field:'fValueTypeCode'
	});
	var fValueTypeShow = $('#fixed_add_plan3').datagrid('getEditor',{
		index:index,
