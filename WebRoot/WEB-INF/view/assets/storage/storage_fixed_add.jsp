...
<div class="window-div">
					...
					<div title="资产登记表" data-options="collapsed:false,collapsible:false" style="overflow:auto;margin-top:10px;">
						<table class="ourtable" border="0" cellpadding="0" cellspacing="0">
							...
							<tr class="trbody">
								...
								<td class="td2">
									<input class="easyui-combobox" id="storage_fixed_fAssClass" name="fAssClass"  data-options="url:'${base}/lookup/lookupsJson?parentCode=GDZCLB&selected=${bean.fAssClass}',method:'post',valueField:'code',textField:'text',editable:false,onSelect:onselectfAssClass" style="width: 200px;">
								</td>
								<td class="td4">&nbsp;</td>
								<td class="td1"><span class="style1">*</span>&nbsp;取得方式</td>
								<td class="td2">
									<input class="easyui-combobox" id="storage_fixed_fGainingMethods" name="fGainingMethod"  data-options="url:'${base}/lookup/lookupsJson?parentCode=QDFS&selected=${bean.fGainingMethod}',method:'get',valueField:'code',textField:'text',editable:false,onSelect:onselectMethod" style="width: 200px;">
								</td>
							</tr>
							<tr class="trbody">
  ...
  //如果选择‘采购’主管方法会弹出页面，让你选择采购计划
    ...
    function StroageFixedAddEditForm(){
    	var fGainingMethod = $("#storage_fixed_fGainingMethods").combobox('getValue');
    	if(fGainingMethod==''){
    		alert('请选择取得方式！');
    		return false;
    	}
    	if(fGainingMethod=='QDFS-01'){
    	...
    	var getPlans = getPlan();
    	var rows = $('#fixed_add_plan1').datagrid('getRows');
    	if(rows!=''){
    		for (var i = 0; i < rows.length; i++) {
        		if(rows[i].fFixedTypeId==''|| rows[i].fFixedTypeId==undefined||rows[i].fFixedTypeId==null||rows[i].fAssName==''||rows[i].fAssName==null||rows[i].fMMode==''||rows[i].fActionDate==''){
	...
	function deleteF1(val){
		...
		function onselectMethod(record){
			if("QDFS-01"==record.code){
				$('#S_facpCode').textbox('enable'); //可用
				$('#S_facpCode').textbox('readonly','false');
				$('#S_facpCode').textbox({prompt:'单击选择'});
				$("#queryAcpCodeId").attr("onclick","queryAcpCode();");
				$("#removeitPlanId").hide();
				$("#appendPlanId").hide();
			}else{
		...
		function queryAcpCode(){
			acceptPlan();
			var fAssType =  $('#S_fAssType').val();
			var win=creatFirstWin('选择-验收单',900,580,'icon-search',"/Storage/selectReceive?fAssType="+fAssType);
		    win.window('open');
		}
