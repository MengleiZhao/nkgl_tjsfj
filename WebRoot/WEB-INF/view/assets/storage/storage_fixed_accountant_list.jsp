...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
		...
		<div class="list-table">
			...
			data-options="collapsible:true,url:'${base}/Storage/accountantEnteringlist?accountantEntering=0&fAssType=${fAssType}',
...
function mouseOut(img) {
	...
	function CZ(val, row) {
		...
		if(row.accountantEntering=='0'){
			return 	'<table><tr style="width: 105px;height:20px"><td style="width: 25px">'
			+'<a href="#" onclick="storage_fixed_update(' + row.fId_S
			+ ')" class="easyui-linkbutton"><img <img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/bulu1.png">' + '</a>'
			+'</td></tr></table>';
		}else{
			if(row.accountantEntering=='1'){
				return 	'<table><tr style="width: 105px;height:20px"><td style="width: 25px">'
				+'<a href="#" onclick="storage_fixed_detail_acc(' + row.fId_S+ ')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/select1.png">' + '</a>'
				+ '</a></td><td style="width: 25px">'
				+'<a href="#" onclick="storage_print(' + row.fId_S
				+ ')" class="easyui-linkbutton"><img <img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/dy1.png">' + '</a>'
				+'</td></tr></table>';
			}
		}
	}
	...
	function storage_fixed_update(id) {
		if(fAssType=='ZCLX-02'){
			var win = creatWin('资产-补录', 720, 580, 'icon-search',"/Storage/accountantEdit/" + id);
			win.window('open');
		}else{
			var win = creatWin('资产-补录', 720, 580, 'icon-search',"/Storage/accountantIntangibleEdit/" + id);
			win.window('open');
		}
	}
	
	function storage_fixed_detail_acc(id){
		if(fAssType=='ZCLX-02'){
	...
	function storage_print(id){
		if(fAssType=='ZCLX-02'){
			window.open(base+"/exportAssets/storageFixed?id="+ id);
		}else{
			window.open(base+"/exportAssets/storageIntangible?id="+ id);
		}
	}
</script>
</body>
