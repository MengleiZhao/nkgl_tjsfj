...
<div class="easyui-layout" fit="true">
	<%-- <div data-options="region:'west',split:false"  style="width:200px;" >
		<ul id="fixedType" class="easyui-tree" data-options="url:'${base}/assetType/tree',animate:true,lines:true" ></ul>
	</div> --%>
	<div data-options="region:'center'" style="background-color: #f0f5f7">
	  <div style="height: 10px;background-color:#f0f5f7 "></div>
			  ...
			  <table class="top-table" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td class="top-table-td2">
						<input id="choose_recefixed_Name" class="easyui-textbox" size="15"  maxlength="10" style="width: 150px;height:25px;"/>
					</td>
					<!-- <td style="width: 8px;"></td>
					<td class="top-table-td1">卡片编号:</td> 
					<td class="top-table-td2">
						<input id="choose_recefixed_Period" class="easyui-textbox" size="15"  maxlength="10" style="width: 150px;height:25px;"/>
					</td> -->
					<td style="width: 8px;"></td>
					<td style="width: 26px;">
		...
		<table id="choose_recefixed_dg" border="0" class="easyui-datagrid"
				data-options="singleSelect:true,collapsible:true,url:'${base}/assetType/allFixedTypeList?assetType=0&fFixedType=${fixedType}',
...
function save(){
	...
	var url = base+'/Rece/getbyAssetid?assetid='+entities;
	$('#Rece_app_config_plan').datagrid('reload',url);
	closeFirstWindow();
}
$('#checkAsset').tagbox().tagbox({
	onRemoveTag:function(value){
		...
		var rows = $('#choose_recefixed_dg').datagrid('getRows');
		for (var i = rows.length-1; i >= 0; i--) {
			if(rows[i].fAssName==value){
				$('#choose_recefixed_dg').datagrid('uncheckRow',i);
			}
		}
	},
});
$('#choose_recefixed_dg').datagrid({
	onCheck:function(rowIndex,rowData){
		...
		//checkAssetid = checkAssetid+rowData.fAssId+',';
	...
	onUncheck:function(rowIndex,rowData){
		...
		//checkAssetidarr = checkAssetidarr.substring(0,checkAssetidarr.length -1);
...
//分页样式调整
$(function(){
	 var addr_tree = $("#fixedType").tree({  
	        url:'${base}/assetType/tree?type=ZCLX-02',  
	        method:"post",  
	        onSelect:function(node){},
	        onLoadSuccess:function(node,data){  
	        	//找到第一条数据
	        	var n = $('#fixedType').tree('find', data[0].id);
		        //调用选中事件
		        $('#fixedType').tree('select', n.target);
	        	var node=$('#fixedType').tree('getSelected');
				$('#choose_recefixed_dg').datagrid('load',{ 
					name:$('#choose_recefixed_Name').val(),
					code:$('#choose_recefixed_Period').val(),
...
function view(){
	var row = $('#choose_recefixed_dg').datagrid('getSelected');
	var selections = $('#choose_recefixed_dg').datagrid('getSelections');
...
function deleteEconomic(){
	var node=$('#fixedType').tree('getSelected');
	var row =$('#choose_recefixed_dg').datagrid('getChecked');
	var selections = $('#choose_recefixed_dg').datagrid('getSelections');
	var da=$('#choose_recefixed_dg').datagrid('getChecked');
	var fids=[15];
	var str;
				...
				$.ajax({ 
					...
					success: function(data){ 
						if(data.success){
							...
							$("#choose_recefixed_dg").datagrid('reload');
						}else{
...
function queryUser(){
	var node=$('#fixedType').tree('getSelected');
	$('#choose_recefixed_dg').datagrid('load',{ 
		fAssName:$('#choose_recefixed_Name').val(),
		fAssCode:$('#choose_recefixed_Period').val(),
		fFixedTypeCode:node.code,
	}); 
}
