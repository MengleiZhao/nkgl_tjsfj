...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
			...
			<table class="top-table" cellpadding="0" cellspacing="0">
				<tr>
					<td class="top-table-search">调拨单号&nbsp;
						<input id="allcoa_fAssAllcoaCode" name="" onchange="CheckYou()" style="width: 150px;height:25px;" class="easyui-textbox"></input>
						&nbsp;&nbsp;审批状态&nbsp;
						<select id="allcoa_fFlowStauts" name="" style="width: 150px;height:25px;" class="easyui-combobox" editable="false">
						...
						&nbsp;&nbsp;申请时间&nbsp;
						<input id="allcoa_fReqTimeBegin" name="" style="width: 150px;height:25px;" class="easyui-datebox" editable="false"></input>
						&nbsp;-&nbsp;
						<input id="allcoa_fReqTimeEnd" name="" style="width: 150px;height:25px;" class="easyui-datebox" editable="false"></input>
						&nbsp;&nbsp;
						<a href="javascript:void(0)"  onclick="queryAlloca();">
						...
						&nbsp;&nbsp;
						<a href="javascript:void(0)"  onclick="alloca_clearTable();">
					...
					<td class="top-table-td2">
						<input id="allcoa_fFlowStauts" name=""  style="width: 150px;height:25px;" class="easyui-textbox"></input>
					</td>
					<td class="top-table-td1">接收部门：</td> 
					<td class="top-table-td2">
						 <input id="allcoa_fReqTimeEnd" name="" style="width: 150px;height:25px;" data-options="" class="easyui-textbox"></input>
					</td>
					<td class="top-table-td1">申请时间：</td> 
					<td class="top-table-td2">
						<input id="allcoa_fReqTimeBegin" name="" style="width: 150px;height:25px;" class="easyui-datebox"></input>
					</td>
					<td style="width: 12px">
					</td>
					<td style="width: 24px;">
						<a href="javascript:void(0)"  onclick="queryAlloca();"><img src="${base}/resource-modality/${themenurl}/button/detail1.png" onmouseover="this.setAttribute('src','${base}/resource-modality/${themenurl}/button/detail2.png')"
								onmouseout="this.setAttribute('src','${base}/resource-modality/${themenurl}/button/detail1.png')"></a>
					</td>
					<td style="width: 12px">
					</td>
					<td >
						<a href="javascript:void(0)"  onclick="alloca_clearTable();"><img src="${base}/resource-modality/${themenurl}/button/qingchu1.png" onmouseover="this.setAttribute('src','${base}/resource-modality/${themenurl}/button/qingchu2.png')"
								onmouseout="this.setAttribute('src','${base}/resource-modality/${themenurl}/button/qingchu1.png')"></a>
					</td>
					<td style="width: 12px">
					</td>
					<td align="right">
						<a href="#" onclick="alloca_add()" ><img src="${base}/resource-modality/${themenurl}/button/xz1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)" ></a>
					</td>
					<td style="width: 14px">
					</td> --%>
				</tr>
		...
		<div class="list-table">
			<table id="allcoa_dg" class="easyui-datagrid"
			data-options="collapsible:true,url:'${base}/Alloca/appJsonPagination',
...
//清除查询条件
function alloca_clearTable() {
	$('#allcoa_fAssAllcoaCode').textbox('setValue',null),
	$('#allcoa_fFlowStauts').combobox('setValue',null),
	$('#allcoa_fReqTimeBegin').datebox('setValue',null),
	$('#allcoa_fReqTimeEnd').datebox('setValue',null),
	queryAlloca();
}
//鼠标移入图片替换
...
function mouseOut(img) {
	...
	function CZ(val, row) {
		if(fs==9){
			return '<table><tr style="width: 105px;height:20px"><td style="width: 25px">'
					+'<a href="#" onclick="detail(' + row.fId_A+ ')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/select1.png">' + '</a>   '
					+'</td></tr></table>';
		}
		if(fs==1){
			return '<table><tr style="width: 105px;height:20px"><td style="width: 25px">'
					...
					+ '<a href="#" onclick="reCall(\'allcoa_dg\' ,'+ row.fId_A+',\'/Alloca/reCall\''+')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/reCall1.png">' + '</a>'
					+'</td></tr></table>';
		}
		return '<table><tr style="width: 105px;height:20px"><td style="width: 25px">'
				+'<a href="#" onclick="detail(' + row.fId_A
				...
				+'</td><td style="width: 25px">'+ '<a href="#" onclick="Alloca_update(' + row.fId_A
				...
				+'</td><td style="width: 25px">'+'<a href="#" onclick="Alloca_delete(' + row.fId_A
	...
	function queryAlloca() {
		$('#allcoa_dg').datagrid('load', {
			fAssAllcoaCode : $('#allcoa_fAssAllcoaCode').val(),
			fFlowStauts : $('#allcoa_fFlowStauts').val(),
			fReqTimeBegin : $('#allcoa_fReqTimeBegin').val(),
			fReqTimeEnd : $('#allcoa_fReqTimeEnd').val(),
		});
	}
	function alloca_add() {
		var win = creatWin('资产调拨申请', 970,580, 'icon-search',"/Alloca/add");
		win.window('open');
	}
	function detail(id) {
			var win = creatWin('资产调拨明细', 970,580, 'icon-search',"/Alloca/detail/" + id);
			win.window('open');
	}
	/* function editCF() {
		var row = $('#allcoa_dg').datagrid('getSelected');
	...
	function Alloca_update(id) {
		//var row = $('#allcoa_dg').datagrid('getSelected');
		var win = creatWin('资产调拨-修改', 970,580, 'icon-search',
				"/Alloca/edit/" + id);
		win.window('open');
	}
