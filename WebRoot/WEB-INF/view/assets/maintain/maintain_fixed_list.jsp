...
<div class="list-div">
	<div style="height: 10px;background-color:#f0f5f7 "></div>
		...
		<div class="list-table">
			<table id="miantain_fixed_dg" class="easyui-datagrid"
			data-options="collapsible:true,url:'${base}/Maintain/jsonPagination',
...
function mouseOut(img) {
	...
	function CZ(val, row) {
		if(fs=='0'||fs=='-1'){
			return 	'<a href="#" onclick="maintain_fixed_detail(' + row.fID+ ')" class="easyui-linkbutton"><img onmouseover="listMouseOver(this)" onmouseout="listMouseOut(this)" src="'+base+'/resource-modality/${themenurl}/list/select1.png">' + '</a>'
					+'&nbsp;&nbsp;'
					+'<a href="#" onclick="maintain_fixed_update(' + row.fID
					...
					+'&nbsp;&nbsp;'
					+'<a href="#" onclick="maintain_fixed_delete(' + row.fID
	...
	function maintain_fixed_detail(id) {
	...
	/* function editCF() {
		var row = $('#miantain_fixed_dg').datagrid('getSelected');
	...
	function maintain_fixed_update(id) {
		//var row = $('#miantain_fixed_dg').datagrid('getSelected');
		var selections = $('#miantain_fixed_dg').datagrid('getSelections');
	...
	function maintain_fixed_delete(id) {
		if (confirm("确认删除吗？")) {
			$.ajax({
				type : 'POST',
