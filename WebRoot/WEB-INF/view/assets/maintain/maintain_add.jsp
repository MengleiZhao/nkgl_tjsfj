...
<form id="maintainAddEditForm" action="${base}/Maintain/save" method="post" data-options="novalidate:true" class="easyui-form" enctype="multipart/form-data">
	<div class="easyui-layout" style="height: 509px;">
		<div class="win-left-div" data-options="region:'west',split:true">
			<div class="win-left-top-div">
		    	...
		    	<input type="hidden" name="fRegStauts" id="m_fRegStauts" value="${bean.fRegStauts}"/>
					...
					<div class="easyui-accordion" style="width:662px;margin-left: 15px;">
							<div title="维修单详情" data-options="iconCls:'icon-xxlb',collapsed:false,collapsible:false" style="overflow:auto;margin-top:10px;">
								...
								<tr class="trbody">
										...
										<td  colspan="4">
											<input id="m_tAssetMainCode" required="required" class="easyui-textbox" type="text" readonly="readonly" name="tAssetMainCode" data-options="prompt:'系统自动生成',validType:'length[1,20]'" value="${bean.tAssetMainCode}" style="width: 555px;color: #f7f7f7;"/> 
										</td>								
									</tr>
									<tr class="trbody">
										<td class="td1"><span class="style1">*</span>资产名称</td>
										<td class="td2">
											<c:if test="${openType=='add'||openType=='edit'}"><a onclick="chooseAss()"></c:if><input class="easyui-textbox" required="required" class="dfinput" id="m_fAssName" name="fAssName"  data-options="prompt:'单击选择维修资产',validType:'length[1,20]'" style="width: 200px" value="${bean.fAssName }"/><c:if test="${openType=='add'||openType=='edit'}"></a></c:if>
										</td>
										<td class="td4">&nbsp;</td>
										<td class="td1"><span class="style1">*</span>资产编号</td>
										<td class="td2">
											<input class="easyui-textbox" required="required" class="dfinput"  id="m_fAssCode" name="fAssCode"  data-options="prompt:'选择资产后系统自动填写',validType:'length[1,20]'" style="width: 200px" value="${bean.fAssCode}"/>
										</td>
									</tr> 
									<tr class="trbody">
										<td class="td1"><span class="style1">*</span>规格型号</td>
										<td class="td2">
											<input class="easyui-textbox" required="required" class="dfinput" id="m_fAssModel" name="fAssModel"  data-options="prompt:'选择资产后系统自动填写',validType:'length[1,20]'" style="width: 200px" value="${bean.fAssModel }"/>
										</td>
										<td class="td4">&nbsp;</td>
										<td class="td1"><span class="style1">*</span>购置日期</td>
										<td class="td2">
											<input class="easyui-datebox" required="required" class="dfinput" id="m_fAcquisitionDate" name="fAcquisitionDate"  data-options="prompt:'选择资产后系统自动填写',validType:'length[1,20]'" style="width: 200px" value="${bean.fAcquisitionDate }"/>
										</td>
									</tr>
									<tr class="trbody">
										<td class="td1"><span class="style1">*</span>是否产生费用</td>
										<td class="td2">
											<input class="easyui-combobox" required="required" class="dfinput" id="m_fMainWhether" name="mainWhether"  data-options="editable:false,panelHeight:'auto',url:'${base}/Maintain/lookupsJson?parentCode=sfcswxfy&selected=${bean.fMainWhether.code}',method:'POST',valueField:'code',textField:'text',editable:false,validType:'selectValid'" value="${bean.fMainWhether.code}" style="width: 200px"/>
										</td>
										<td class="td4">&nbsp;</td>
										<td class="td1"><span class="style1">*</span>报修时间</td>
										<td class="td2">
											<input class="easyui-datebox" required="required" class="dfinput" id="m_fRepairTime" name="fRepairTime"  data-options="prompt:'',validType:'length[1,20]'" style="width: 200px" value="${bean.fRepairTime }"/>
										</td>
									</tr>
									<tr class="trbody">
										<td class="td1"><span class="style1">*</span>申请部门</td>
										<td class="td2">
											<input class="easyui-textbox" required="required" class="dfinput" readonly="readonly" id="m_fMainDept" name="fMainDept"  data-options="prompt:'',validType:'length[1,25]'" style="width: 200px" value="${bean.fMainDept }"/>
										</td>
										<td class="td4">&nbsp;</td>
										<td class="td1"><span class="style1">*</span>申请人</td>
										<td class="td2">
											<input class="easyui-textbox" required="required" class="dfinput" readonly="readonly" id="m_fMainUser" name="fMainUser"  data-options="validType:'length[1,20]'" style="width: 200px" value="${bean.fMainUser}"/>
										</td>
									</tr>
									<tr  hidden="hidden" id="choose1">
										<%-- <td class="td1"><span class="style1">*</span>预算指标名称</td>
										<td class="td2">
											<c:if test="${openType=='add'||openType=='edit'}"><a onclick="chooseFBIndex()"></c:if><input class="easyui-textbox"  class="dfinput"  id="F_fBudgetIndexName" name="fBIndexName"  data-options="prompt:'单击选择指标',validType:'length[1,20]'" style="width: 200px" value="${bean.fBIndexName}"/><c:if test="${openType=='add'||openType=='edit'}"></a></c:if>
										</td> --%>
										<td class="td1"><span class="style1">*</span>维修费用</td>
										<td class="td2">
											<input class="easyui-numberbox"  class="dfinput"  id="m_fMainAmountr" name="fMainAmount"  data-options="icons: [{iconCls:'icon-yuan'}],validType:'length[1,20]',precision:2" style="width: 200px" value="${bean.fMainAmount}"/>
										</td>
										<td class="td4">&nbsp;</td>
									</tr>
									<tr style="height: 70px;">
										...
										<td  colspan="4">
											<%-- <input class="easyui-textbox" data-options="validType:'length[1,200]',multiline:true"
											 id="m_fFaultRemark" name="fFaultRemark" style="width: 555px;height:70px" value="${bean.fFaultRemark}"> --%>  
											<textarea name="fFaultRemark"  id="m_fFaultRemark"  class="textbox-text" oninput="textareaNum(this,'textareaNum1')"
											autocomplete="off"  style="width: 555px;height:70px;resize:none">${bean.fFaultRemark }</textarea>
										</td>
									</tr>	
									<c:if test="${openType=='add'||openType=='edit'}">
										<tr>
											...
											可输入剩余数：<span id="textareaNum1" class="200">
												<c:if test="${empty bean.fFaultRemark}">200</c:if>
												<c:if test="${!empty bean.fFaultRemark}">${200-bean.fFaultRemark.length()}</c:if>
											</span>
											</td>
										</tr>
									</c:if>	
			...
			<div class="win-left-bottom-div">
				<c:if test="${openType=='add'||openType=='edit'}">
					<a href="javascript:void(0)" onclick="maintainAddEditForm();">
						<img src="${base}/resource-modality/${themenurl}/button/zhanchun1.png" onMouseOver="mouseOver(this)" onMouseOut="mouseOut(this)">
					</a>&nbsp;&nbsp;&nbsp;
					<a href="javascript:void(0)" onclick="maintainFormSS()">
	...
	$(document).ready(function (){
		var sel2=$('#m_fMainWhether').combobox().combobox('getValue');
    	if(sel2=="sfcswxfy-01"){
    		$('#F_fBudgetIndexName').attr("required",true);
    		$('#choose1').show();
		}else{
    		$('#m_fMainAmountr').numberbox().numberbox('setValue',0);
    ...
    var c=0;
		...
		//弹出选择资产的页面
		function chooseAss(){
			var win=creatFirstWin('选择需要维修资产',970,630,'icon-search','/Maintain/chooseAss');
			win.window('open');
		}
		function quota_DC(){
			//var node=$('#miantain_fixed_dg').datagrid('getSelected');
