  ...
  <div data-options="region:'center'">
  	<div style="height: 100%; width:98%; margin-left: 1%;">
		<table id="pro_dg"  class="easyui-datagrid"
			data-options="collapsible:true,url:'${base}/cockDetail/indexDataList?typeStr=budget&id=${departId}&indexName=${typeNum}&year=${year}',
			...
			<thead>
				<tr>
					...
					<th data-options="field:'zcAmount',align:'right',formatter:zcAmount" style="width: 15%">执行金额(万元)</th>
					...
					<th data-options="field:'operation',align:'center',formatter: cockpit_zbdg_CZ" style="width: 8%">操作</th>
				</tr>
			</thead>
		</table>
	</div>
...
//点击 返回（生成饼图）
function goback(){
	var win = parent.creatCockFirstWin('单位总预算', 677,400, 'icon-search','/cockDetail/tBudget?year='+${year}+'&departId='+queryDepartId);
	win.window('open');
}
//操作栏创建
function cockpit_zbdg_CZ(val, row) {
...
function getUse(val,row) {
	var syAmount=row.pfAmount-row.syAmount-row.djAmount;
	if(syAmount<0){
		syAmount=-syAmount;
	}
	return parseFloat((syAmount/row.pfAmount)*100).toFixed(2)+"%";
}
//执行金额
function zcAmount(val, row) {
	return parseFloat(row.pfAmount-row.djAmount-row.syAmount).toFixed(2);
}
//鼠标悬浮单元格提示信息  
