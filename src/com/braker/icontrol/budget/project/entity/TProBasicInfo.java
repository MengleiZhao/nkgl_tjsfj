...
public class TProBasicInfo extends BaseEntityEmpty implements java.io.Serializable,EntityDao,CheckEntity {
	...
	/*审批状态    备选库 0：暂存11：提交，待审核-11：已退回（所有流程如驳回，则直接回退至申请人）12：审核中（2->8 系统支持8级审批）19：已审核
	上报库 一上   21：提交，待审核-21：已退回（所有流程如驳回，则直接回退至申请人）22：审核中（2->8 系统支持8级审批）29：已审核
	上报库 二上   31：提交，待审核-31：已退回（所有流程如驳回，则直接回退至申请人）32：审核中（2->8 系统支持8级审批）39：已审核*/
	...
	private String stauts;//项目状态1保存99删除
	...
	private BigDecimal pfAmount;//批复金额(数据库中没有，用于前台)
	...
	public void setFStauts(String stauts) {
		this.stauts = stauts;
	}
	@Transient
